<?php
	...
	if (!getimagesize ($_FILES["image"]["tmp_name"])) {
		header ("Location: ../userPage.php?message=Загружаемый файл должен быть изображением");
		exit;
	}
    ...
    elseif ($arr["mime"] == "image/bmp") { // bmp
		...
		header ("Location: ../userPage.php?message=Поддерживаемые форматы изображения: jpeg, jpg, png и bmp");
		exit;
	}
	...
	if($filesize > 2) {
		header ("Location: ../userPage.php?message=Изображение не должно превышать 2 мб");
	}
	...
	if (!move_uploaded_file ($_FILES["image"]["tmp_name"], "../images/before/" . $image_name)) {
		header ("Location: ../userPage.php?message=Произошла ошибка с сохранением вашего изображения");
		exit;
	}
	...
	header ("Location: ../userPage.php?message=Заявка создана#auth");
	exit;
?>